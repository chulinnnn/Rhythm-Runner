...
#pragma warning restore 162
    ...
    private bool BindExistingMenu(GameObject existing)
    {
        ...
        BindSlider(existing.transform, "Root/SettingsPanel/Card/SettingsContent/VisualAssistStrengthRow/Slider", VisualAssistKey, delegate(float value)
        {
    ...
    private void BindToggle(Transform root, string path, string key)
    {
        Transform child = root.Find(path);
        Toggle toggle = child != null ? child.GetComponent<Toggle>() : null;
        if (toggle == null)
        {
            return;
        }
        ...
        toggle.onValueChanged.AddListener(delegate(bool enabled)
        {
            ...
            if (image != null)
            {
                image.color = enabled ? new Color(0.32f, 0.82f, 0.38f) : new Color(0.55f, 0.58f, 0.62f);
            }
        });
    }
