...
// Shared data model for OceanRhythm fish, beat cards, bucket rewards, and PlayerPrefs inventory.
// OceanRhythm 的共享数据模型：鱼、节奏卡、桶装饰奖励和 PlayerPrefs 背包存档。
//
// Runtime data can change counts/unlocks/slot assignments; visual presentation remains owned by UI hierarchy.
// Runtime 可以改变数量、解锁和槽位配置；视觉呈现仍由 UI Hierarchy 控制。
