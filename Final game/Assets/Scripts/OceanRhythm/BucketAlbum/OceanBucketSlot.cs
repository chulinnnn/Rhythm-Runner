...
// Editable bucket slot component for the Bucket Album preview.
// Bucket Album 预览中可编辑的桶槽位组件。
//
// Build only creates missing child pieces and fallback visuals. Existing slot images, labels,
// positions, and styling should stay under Hierarchy control.
// Build 只补缺失子节点和兜底视觉；已有槽位图片、文字、位置和样式应由 Hierarchy 控制。
