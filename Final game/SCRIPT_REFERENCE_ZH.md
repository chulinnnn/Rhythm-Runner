...
- 类型：`场景挂载`、`自动入口`
- 所属场景：`Start`
- 用途：Start 主菜单总控制器。绑定可编辑的 `StartMenuCanvas`，给三个模式卡片、设置面板、家长说明等 UI 绑定按钮行为，并负责跳转到 `OceanRhythm`、`VerticalRunner`、`AdvancedRunner`。
- 关键依赖：`RuntimeScenePolicy`、`SceneTransitionManager`、`StartMenuCanvas` 下的 UI 对象命名。
- 是否建议改名：不建议。它是 Unity 场景组件，也是自动入口脚本。

### `Assets/Scripts/Start/Leaderboard/LeaderboardBootstrap.cs`

- 类型：`自动入口`
- 所属场景：`Start`
- 用途：进入 Start 类场景时自动创建排行榜系统。它会寻找名为 `phbtn` 的按钮，存在时创建 `LeaderboardUI`。
- 关键依赖：`LeaderboardUI`、`LeaderboardManager`、名为 `phbtn` 的按钮对象。
- 是否建议改名：不建议。它依赖 Unity 自动初始化和 Start UI 命名。

### `Assets/Scripts/Start/Leaderboard/LeaderboardManager.cs`

- 类型：`数据/辅助`
- 所属模块：排行榜
- 用途：用 `PlayerPrefs` 保存和读取 Easy/Hard 排行榜分数。
- 关键依赖：`LeaderboardMode`、当前场景名。
...
- 类型：`运行时创建`
- 所属场景：`Start`
- 用途：运行时创建并控制排行榜弹窗 UI，把行为绑定到 `phbtn`，显示 Easy/Hard 分数列表。
- 关键依赖：`LeaderboardBootstrap`、`LeaderboardManager`、`phbtn`、Unity UI/EventSystem。
- 是否建议改名：不建议。`LeaderboardBootstrap` 会按类型创建它。
